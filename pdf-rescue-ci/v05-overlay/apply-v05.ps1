...
$text = $text -replace 'InspectorSecurity.Text = \"Not checked\";\r?\n\s*HealthText.Text = \"Not checked\";', 'InspectorSecurity.Text = "Not checked";' + "`r`n" + '            InspectorFeatures.Text = "Run PDF Doctor to inspect";' + "`r`n" + '            HealthText.Text = "Not checked";'
...
$c = $c.Replace('Console.WriteLine($"Encrypted: {(report.Inspection.IsEncrypted ? "Yes" : "No")}");', 'Console.WriteLine($"Encrypted: {(report.Inspection.IsEncrypted ? "Yes" : "No")}");' + "`r`n" + '            var features = report.Inspection.Features ?? PdfFeatureSummary.Empty;' + "`r`n" + '            Console.WriteLine($"Features: images={features.ImageCount}; imagePages={features.PagesWithImages}; forms={features.HasForms}; js={features.HasJavaScript}; signatures={features.HasDigitalSignatures}; linearized={features.IsLinearized}");')
