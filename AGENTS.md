...
- When the user says "push," treat the task as complete only after the full production deployment succeeds. This includes committing and pushing the intended files to `main`, monitoring the GitHub Actions deployment to completion, confirming every included database migration was applied on production, verifying the VPS is running the pushed revision, checking API and nginx service health, running relevant production smoke tests, and reviewing fresh logs for migration errors, backend exceptions, and HTTP 5xx responses. Do not report a push as complete merely because Git accepted it. If deployment or migration verification fails, keep investigating within scope and clearly report that production is not yet complete.
...
- WhatsApp retry note, July 18, 2026: production challenge #50 for `skgasante@gmail.com` stayed pending after a WhatsApp attempt because there were still no nginx `POST /api/webhooks/whatsapp` entries from Meta and no `whatsapp_webhook_events`. Graph readback initially showed `/{WABA_ID}/subscribed_apps` empty, so the WABA was re-subscribed with the configured production system-user token; Graph returned `{"success": true}` and an unfiltered `/{WABA_ID}/subscribed_apps` readback showed app `RealMindX` (`2009554023007456`). The filtered `?app_id=...&fields=...` readback can misleadingly return `data: []`; use the unfiltered readback when checking. Production phone number `816622104862309` still reported `platform_type=ON_PREMISE` and `code_verification_status=NOT_VERIFIED`; if inbound messages still do not arrive after re-subscribe, inspect that Meta phone-number platform/verification state next. A non-secret deploy safety script now runs `realmindx-site/scripts/ensure_whatsapp_subscription.py` after migrations to idempotently re-subscribe the configured WABA.
