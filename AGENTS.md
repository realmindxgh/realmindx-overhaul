...
- Follow-up WhatsApp webhook note, July 16, 2026: after the app-level subscription was active, inbound WhatsApp challenge messages still did not reach RealMindX. Production showed contact challenge #43 pending with no `whatsapp_webhook_events`, no nginx `POST /api/webhooks/whatsapp`, and no API webhook POSTs. Graph `GET /{WABA_ID}/subscribed_apps` using the app token returned `(#200) You do not have permission to access this field` for both observed WABA IDs (`1095294615457846` RealMindX WABA, `1526957368306594` test WABA). Treat the likely missing piece as WABA-level app subscription / system-user token authorization, not backend parsing.
- Safe Meta-side fix for the above: subscribe the RealMindX WABA to the app using a system-user permanent token with `whatsapp_business_management` (and normally `whatsapp_business_messaging`) access, or use Meta UI if it exposes “Subscribe app” for the WABA. The Graph action is `POST https://graph.facebook.com/v25.0/1095294615457846/subscribed_apps` with `access_token=<SYSTEM_USER_TOKEN>`. Do not paste the token into chat or logs. If the user adds GitHub secrets, use `WHATSAPP_ACCESS_TOKEN`, `WHATSAPP_PHONE_NUMBER_ID` (`816622104862309` from the WhatsApp Manager screenshot), and `WHATSAPP_BUSINESS_ACCOUNT_ID` (`1095294615457846`) so the deploy workflow can persist them into production `.env`.
