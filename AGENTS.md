...
- When the user says "push," treat the task as complete once the GitHub Actions deployment run succeeds. The workflow itself runs the regression suite, builds the frontend, applies migrations, restarts services, reloads nginx, and runs post-deploy SEO smoke checks, and it emails a failure alert if anything breaks. Confirming the workflow run concluded `success` (via `gh run view <run-id>`) is therefore sufficient verification. Do not report a push as complete merely because Git accepted it, but also do not perform extra manual VPS verification as a routine step. If the workflow fails or reports a failure alert, keep investigating within scope and clearly report that production is not yet complete.
